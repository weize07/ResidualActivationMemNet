...
class ResiNetwork(object):
    ...
    def __init__(self, sizes, actfuncs):
    ...
    def feedforward(self, xs, q):
        ...
        self.load_memory(xs)
        for b, w, r, f in zip(self.biases, self.weights, self.resi, self.actfuncs):
            q = f.forward(np.dot(w, q) + b + r)
        self.clear_memory()
        return q
        ...
        for x in mem:
            ...
            for b, w, r, f in zip(self.biases, self.weights, self.resi, self.actfuncs):
                ...
                for j in range(len(m)):
                    if m[j][0] <= 0:
                        m[j][0] = 0
                # self.resi[i] = r * self.FORGET_RATE + self.filter_mem(x * self.LEARNING_RATE)
                self.resi[i] = r * self.FORGET_RATE + m * self.LEARNING_RATE
                i += 1

    def filter_mem(self, m):
...
def main():
    ...
    # exit()
    story_maxlen = 0
    for story, _, _ in train + test:
        for s in story:
            story_maxlen = max(len(s), story_maxlen)
    ...
    print('vocab = {}'.format(vocab), 'size: ', len(vocab))
    ...
    relu = Relu()
    soft = Softmax()
    model = ResiNetwork([len(x[0][0]), 200, 100, len(y[0])], [relu, relu, soft])
    model.SGD(train_ds, 1000, 5, 0.01, test_ds)
